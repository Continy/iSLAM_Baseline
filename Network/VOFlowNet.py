...
class VOFlowRes(nn.Module):
    ...
    def __feature_embedding(self):
        ...
        if not self.down_scale:
            layers.append(
                self.__make_layer(BasicBlock, outputnums[0], blocknums[0], 2,
                                  1, 1))  # (160 x 112)
            layers.append(
                self.__make_layer(BasicBlock, outputnums[1], blocknums[1], 2,
                                  1, 1))  # (80 x 56)

        layers.append(
            self.__make_layer(BasicBlock, outputnums[2], blocknums[2], 2, 1,
                              1))  # 28 x 40
        layers.append(
            self.__make_layer(BasicBlock, outputnums[3], blocknums[3], 2, 1,
                              1))  # 14 x 20
        layers.append(
            self.__make_layer(BasicBlock, outputnums[4], blocknums[4], 2, 1,
                              1))  # 7 x 10
        layers.append(
            self.__make_layer(BasicBlock, outputnums[5], blocknums[5], 2, 1,
                              1))  # 4 x 5
        layers.append(
            self.__make_layer(BasicBlock, outputnums[6], blocknums[6], 2, 1,
                              1))  # 2 x 3
