...
class TartanVO(nn.Module):
    ...
    def __init__(self,
                 ...
                 flow_model_name=None,
                 stereo_model_name=None,
                 device_id=0,
                 correct_scale=True,
                 fix_parts=(),
                 use_kitti_coord=True):
        ...
        self.pose_std = torch.tensor([0.13, 0.13, 0.13, 0.013, 0.013,
                                      0.013]).cuda(self.device_id)
    ...
    def load_model(self, model, modelname):
        pretrain_dict = torch.load(modelname,
        ...
        for k, v in pretrain_dict.items():
            for kk, vv in model_dict.items():
                if (k.endswith(kk)
                        or kk.endswith(k)) and v.size() == vv.size():
                    loadin_dict[kk] = v
        ...
        for kk in model_dict.keys():
            if kk not in loadin_dict:
                print("! [load_model] Key {} in model but not in {}!".format(
                    k, modelname))
    ...
    def forward(self, sample, is_train=True, given_scale=None):
        ...
        with torch.set_grad_enabled(is_train):
            ...
            intrinsic = sample['intrinsic'].cuda(self.device_id,
                                                 non_blocking=nb)

            img0_norm = sample['img0_norm'].cuda(self.device_id,
                                                 non_blocking=nb)
            img0_r_norm = sample['img0_r_norm'].cuda(self.device_id,
                                                     non_blocking=nb)
            ...
            ############################## forward vonet ######################################################################
            flow, disp, pose = self.vonet(img0, img1, img0_norm, img0_r_norm,
                                          intrinsic)
            ...
            if given_scale is not None:
                trans = torch.nn.functional.normalize(
                    pose[:, :3], dim=1) * given_scale.view(-1, 1)
                ...
                pose_ENU_SE3 = tartan2kitti_pypose(
                    pose)  # convert to ENU coordinate
                ...
                for i in range(pose.shape[0]):
                    ...
                    disp_th_dict = {'kitti': 5, 'v1': 1, 'euroc': 1, 'v2': 1}
                    s, z, m, dm = scale_from_disp_flow(
                        disp[i],
                        flow[i],
                        pose_ENU_SE3[i],
                        fx,
                        ...
                        baseline[i],
                        mask=edge[i],
                        disp_th=disp_th_dict[sample['datatype'][i]])
                    scale.append(s)
                    mask.append(m)
                    depth.append(z)
                ...
                trans = torch.nn.functional.normalize(
                    pose[:, :3], dim=1) * scale.view(-1, 1)
                pose = torch.cat([trans, pose[:, 3:]], dim=1)

            else:
                ############################## recover scale from GT ######################################################################
                motion_tar = sample['motion']
                scale = torch.norm(motion_tar[:, :3],
                                   dim=1).cuda(self.device_id)
